...
public interface FoodService 
{
	...
	List<FoodVO> selectAllMenu(int fNo);							// 특정 fNo 값을 가진 메뉴들을 조회하는 메서드
	List<FoodVO> selectAll(SearchVO searchVO);					// 조건에 따라 FoodVO 목록을 조회하는 메서드
	...
	double selectAvg_foodreview(int fNo); 					// 평점을 소수점으로 받는 메서드 
	int selectAllCount_foodreview(int fNo);
	int selectCount_foodreview(FoodReviewVO foodreviewVO);
	int insert_report(Food_ReportVO food_reportVO);			// 식당 신고 
	int insert_review_report(Food_Review_ReportVO food_review_reportVO);		// 식당 신고 
	...
	int delete_menu(int food_menu_number);
}